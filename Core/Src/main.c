...
void setServo(int inData) {
	htim1.Instance->CCR1 = map(inData, 0, 100, 700, 2500);//inData * 10 + 1000;
}
/* USER CODE END 0 */
  	...
  	if(HAL_GetTick() - tick > 1000L) {
  		...
  		if(__HAL_TIM_IS_TIM_COUNTING_DOWN(&htim3)) {
			...
			if((TIM3->SMCR & 0x03) == 0x03) {
				speed = diff * 60 / 4 / OneRotationPulses;
			}
			else {
				speed = diff * 60 / 2 / OneRotationPulses;
			}

			tick = HAL_GetTick();
			cnt1 = TIM3->CNT;

			setServo(map(cnt1, 0, 65535, 0, 100));
  	}
  }
  /* USER CODE END 3 */
}
